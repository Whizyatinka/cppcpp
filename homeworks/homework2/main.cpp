...
int main() {
    ...
    while (window.isOpen()) {
        ...
        // 5 _ Отрисовка графика y = -x (красная линия)
        drawGraph(window, [](float x) { return -x; }, -10, 10, 30, 10, sf::Color::Red);

        // Отрисовка вертикальной линии x = -3 (синяя линия)
        sf::VertexArray verticalLine(sf::Lines, 2);
        verticalLine[0].position = sf::Vector2f(400 + (-3) * 30, 50);
        verticalLine[0].color = sf::Color::Blue;
        verticalLine[1].position = sf::Vector2f(400 + (-3) * 30, 550);
